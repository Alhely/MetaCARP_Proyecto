...
Parámetro $p_{\text{inter}}$ & Fijo $0.4$--$0.5$ por MH          & \textbf{No existe} \\
...
Config base por MH       & Canónica fija                         & Canónica fija (misma) \\
Corridas por MH          & $115$ ($5$ reps $\times 23$)          & $115$ ($5$ reps $\times 23$) \\
...
TS simple    & \texttt{tabu\_tenure}            & $25$    &
  \texttt{tam\_vecindario}: $40$\textsuperscript{‡} \\
RTS          & \texttt{factor\_aumento}         & $1.2$   &
  \texttt{factor\_reduccion}: $0.95$\textsuperscript{‡} \\
ABC simple   & \texttt{num\_fuentes}            & $30$    &
  \texttt{limite\_abandono}: $60$\textsuperscript{‡} \\
Cuckoo       & \texttt{pa\_abandono}            & $0.15$  &
  \texttt{beta\_levy}: $1.3$\textsuperscript{‡}; nidos: instance-aware \\
\bottomrule
\end{tabular}
\\[4pt]
\footnotesize
\textsuperscript{†}Reheat acotado SA: \texttt{patience=10},
\texttt{reheat\_factor=0.5}, \texttt{max\_reheats\_sin\_mejora=10}.
Véase la Nota~\ref{nota:reheat-acotado-binario-capacidad}.
\textsuperscript{‡}Segundo knob calibrado por MH (común a los tres approaches;
calibrado en el Experimento~8).
\end{table}

\paragraph{Corridas totales.}
Una única configuración por MH $\times$ 5 repeticiones $\times$ 23 instancias
$= 115$ corridas por MH ($575$ en total). Sin fase de calibración.
...
Reheat SA            & Default (sin tope)   & Acotado (\texttt{max\_reheats=10})  & Acotado (\texttt{max\_reheats=10}) \\
Config por MH        & ---                  & Canónica fija                       & Canónica fija (misma) \\
\bottomrule
\end{tabular}
\end{table}
...
Cuckoo       & \texttt{pa\_abandono}    & 0.15 &
  19.57 & 18.99 & 1/23 \\
\midrule
\textbf{Media} & --- & --- & \textbf{16.60} & --- & 10/115 \\
\bottomrule
\end{tabular}
\\[4pt]
\footnotesize
...
MH &
  ...
  Interpretación \\
\midrule
SA        & 2.89  & 20.36 & $+17.47$ & Exp.~8 mucho mejor \\
TS simple & 5.76  & 18.41 & $+12.65$ & Exp.~8 mucho mejor \\
RTS       & 4.97  & 14.45 & $+9.48$  & Exp.~8 mucho mejor \\
ABC       & 9.64  & 10.22 & $+0.58$  & equivalentes \\
Cuckoo    & 19.13 & 19.57 & $+0.44$  & equivalentes \\
\midrule
\textbf{Media} &
  \textbf{8.48} & \textbf{16.60} & $+8.12$ & Exp.~8 mejor \\
\bottomrule
\end{tabular}
\\[4pt]
\footnotesize
...
mejor en todas) con una diferencia media de $+8.12$\,pp. El selector
...
búsqueda local: SA pasa de 2.89\,\% a 20.36\,\% ($+17.47$\,pp), TS de
5.76\,\% a 18.41\,\% y RTS de 4.97\,\% a 14.45\,\%. En las poblacionales
(ABC, Cuckoo) la diferencia es marginal ($+0.58$ y $+0.44$\,pp), pero
