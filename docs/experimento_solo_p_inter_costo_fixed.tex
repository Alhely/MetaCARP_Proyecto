...
\item \textbf{ABC simple}: límite de abandono de fuentes proporcional
  al número de fuentes y a $n_\tau$.
...
\begin{itemize}
  ...
  \item \texttt{max\_reheats\_sin\_mejora=5}: máximo de reheats
    consecutivos sin mejorar el mejor global antes de parar.
\end{itemize}
...
150\,s (verificado experimentalmente). Los parámetros de reheat \emph{no
se calibran}: se fijan iguales para los tres valores de $\alpha$ del
grid, de modo que la única variable del grid SA sea efectivamente
$\alpha$.
...
Cuckoo       & 0.1 & \texttt{pa\_abandono}   & 0.15 &
  19.91 & 18.18 & 0/23 \\
\midrule
\textbf{Media} & --- & --- & --- & \textbf{10.21} & --- & 32/115 \\
\bottomrule
\end{tabular}
\\[4pt]
\footnotesize
* Configuración ganadora de la fase 1; leída de \texttt{mejor\_config.json}.
La última columna de la fila \textbf{Media} agrega los aciertos de BKS
como pares (MH, instancia) sobre el total $5 \times 23$ por MH.
\end{table}
...
\midrule
SA        & 0.90  & 0.57 & 3.21  & $+2.31$  \\
TS simple & 4.48  & 1.08 & 8.40  & $+3.92$  \\
RTS       & 3.04  & 1.18 & 4.89  & $+1.85$  \\
ABC       & 9.75  & 8.30 & 14.66 & $+4.91$  \\
Cuckoo    & 8.20  & 4.25 & 19.91 & $+11.71$ \\
\midrule
\textbf{Media} & 5.27 & 3.08 & \textbf{10.21} & $+4.94$ \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Interpretación de los resultados.}
La hipótesis preveía que \texttt{solo\_p\_inter} alcanzaría un gap
\emph{comparable o inferior} a R3. Los datos la \textbf{refutan}: el
approach simple es peor que R3 en las cinco MH ($\Delta$ medio
$+4.94$ puntos) y muy por detrás de R5. Tres lecturas se desprenden:

\begin{enumerate}
\item \textbf{La complejidad sí aporta valor, no solo compensaba el
  evaluador roto.} Bajo el evaluador corregido de forma nativa, los
  mecanismos del primer ciclo (Path Relinking, kick reactivo,
  presupuesto ampliado) siguen mejorando claramente sobre el selector
  desnudo. La corrección del evaluador es necesaria para una
  \emph{medición} fiable, pero no sustituye a la intensificación.
...
\item \textbf{Jerarquía de MH como línea de base.} Con el approach más
  simple, SA (3.21\,\%) y RTS (4.89\,\%) son claramente superiores a las
  metaheurísticas poblacionales (ABC 14.66\,\%, Cuckoo 19.91\,\%), que
  necesitan los componentes adicionales para ser competitivas.
\end{enumerate}

En consecuencia, el resultado fija una \textbf{línea de base limpia}:
todo mecanismo añadido en los siguientes approaches debe justificarse
...
fundamentado en el evaluador greedy integrado de forma nativa. Sus
contribuciones metodológicas son las siguientes, independientemente
...
\item \textbf{Calibración del knob $p_{\text{inter}}$ por MH.}
  El grid search en dos fases cuantifica el valor óptimo de
  $p_{\text{inter}}$ para cada metaheurística bajo el evaluador
  correcto. Este dato es independiente y complementario al obtenido en
  los experimentos R4 y R5 del primer ciclo (donde el evaluador greedy
  se aplicó como monkey-patch sobre una búsqueda ya diseñada con el
  evaluador canónico).
