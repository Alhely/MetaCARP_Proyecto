...
% ---------------------------------------------------------------------
% Capitulo de Experimentacion para tesis MetaCARP.
% Nivel: maestria, rigor completo. Narrativa cronologica, incluyendo
% fracasos y descubrimientos metodologicos.
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{Capitulo de Experimentacion --- MetaCARP}
\fancyhead[R]{\thepage}
...
% =====================================================================
\title{\textbf{Capitulo X --- Experimentacion}\\[6pt]
\large Diseno, ejecucion y analisis del programa experimental sobre el
proyecto MetaCARP}
\author{}
\date{Mayo 2026}
% =====================================================================
...
\item Una \textbf{construccion inicial heuristica} (Path-Scanning
  clasico de Golden, DeArmon y Baker, 1983) combinada con la
...
benchmarks GDB y KSHS, ampliamente utilizados en la literatura de
CARP. La Tabla~\ref{tab:instancias} resume sus caracteristicas.
...
SA es una metaheuristica de trayectoria que acepta movimientos
deteriorantes con probabilidad $\exp(-\Delta/T)$, donde $T$ es la
...
TS Simple mantiene una lista \emph{tabu} de movimientos prohibidos
...
RTS extiende TS Simple con dos mecanismos adicionales:
...
ABC es una metaheuristica poblacional. Mantiene un conjunto de
...
Cuckoo Search mantiene un conjunto de \texttt{num\_nidos} soluciones.
...
activa de la metaheuristica desde Fialho et al. (2008). El esquema
elegido es \emph{Probability Matching} (PM): la probabilidad asignada a
...
\emph{Path Relinking} (PR) es un mecanismo de intensificacion que
...
de Path-Scanning de Golden, DeArmon y Baker (1983) y (ii) el evaluador
...
Path-Scanning es la heuristica constructiva clasica para CARP. El
...
[\emph{Pendiente. El grid R1--R5 (2875 corridas adicionales) esta
programado para ejecucion. Esta seccion se completara con la tabla
maestra cuando los resultados esten disponibles.}]
...
experimento del primer ciclo (1--6) y los resultados parciales del
ciclo siguiente (7).

\begin{table}[h]
\centering
\caption{Comparativa de gap medio (\%) por MH y experimento.}
\label{tab:comparativa-global}
\small
\begin{tabular}{lrrrrrr}
\toprule
MH & strict & aos & pr & p\_inter\_pr & budget & wsgreedy* \\
\midrule
SA      & 41.16 & 41.06 & 38.78 & 31.47 & 30.56 & \textbf{0.00} \\
TS      & 30.14 & 40.31 & 38.81 & 32.51 & 30.59 & \textbf{0.00} \\
RTS     & 29.85 & 40.00 & 37.82 & 32.25 & 31.29 & \textbf{0.00} \\
ABC     & 41.50 & 41.80 & 34.35 & 34.89 & 34.90 & 1.32 \\
Cuckoo  & 40.39 & 40.26 & 36.14 & 35.69 & 33.89 & 7.08 \\
\midrule
\textbf{Media} & 36.61 & 40.69 & 37.18 & 33.36 & 32.25 & 1.68 \\
\bottomrule
\end{tabular}
\\[4pt]
\footnotesize
*Resultados parciales (smoke test sobre 2 instancias por MH; grid completo pendiente).
\end{table}

\subsection{Tabla de tiempo medio por corrida}

La Tabla~\ref{tab:tiempo-medio} resume el costo computacional por
experimento. El presupuesto $\times 25$ multiplica el tiempo por
$5$--$15 \times$. El experimento \texttt{warmstart\_greedy}, gracias a
una mejor inicializacion, recupera tiempos comparables a los del
experimento strict.

\begin{table}[h]
\centering
\caption{Tiempo medio por corrida (segundos).}
\label{tab:tiempo-medio}
\small
\begin{tabular}{lrrrrrr}
\toprule
MH & strict & aos & pr & pip & budget & wsgreedy \\
\midrule
SA      &  14.0 &  17.1 &  32.8 &  44.8 & 249.6 &   6.4 \\
TS      &   3.3 &   3.3 &  13.3 &  20.4 & 309.3 &  25.7 \\
RTS     &   5.2 &   7.1 &  22.3 &  30.6 & 373.6 &  27.6 \\
ABC     &   3.0 &   2.9 &  21.4 &  29.3 &  40.2 &   5.2 \\
Cuckoo  &   3.4 &   3.0 &  15.2 &  19.9 & 162.7 &   6.9 \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Ranking final del primer ciclo}

Bajo el evaluador canonico (experimentos 1--6), el ranking de gap
medio agregado sobre las 5 MHs es:

\begin{enumerate}
\item budget: 32.25\%
\item p\_inter\_pr: 33.36\%
\item strict: 36.61\%
\item pr: 37.18\%
\item aos: 40.69\%
\end{enumerate}

Este ranking permanece valido para comparar experimentos entre si,
pero NO refleja la calidad fisica de las soluciones. Una vez
disponibles los resultados del segundo ciclo (R1--R5 y
warmstart\_greedy completo), el ranking real se podra calcular.
...
\begin{enumerate}
\item Un selector binario estricto inhibe la exploracion en MHs de
  trayectoria (TS Simple, RTS). El selector probabilistico moderado
  con $p_{\text{inter}} = 0.20$ es robusto en las 5 MHs.
\item Path Relinking truncado aporta ganancia significativa en MHs
  poblacionales (ABC, Cuckoo) donde existe diversidad genuina entre
  origen y guia.
\item Aumentar el presupuesto de iteraciones por un factor $\times 25$
  produce mejora marginal cuando el cuello de botella esta en otra
  parte de la cadena.
\item La heuristica clasica Path-Scanning de Golden, DeArmon y Baker
  (1983) es competitiva con BKS en al menos $2/23$ instancias del
  benchmark gdb/kshs cuando se evalua con orientacion correcta.
\end{enumerate}
...
\begin{enumerate}
\item Completar las grids de \texttt{warmstart\_greedy} y R1--R5 para
  validar las hipotesis sobre el conjunto completo de 23 instancias.
\item Comparar el evaluador greedy local con el optimo de orientacion
  global (programacion dinamica O(n) por ruta); cuantificar la
  diferencia esperada en las 23 instancias.
\item Extender el benchmark a instancias mayores (egl-s4, val9B) y
  verificar la robustez de las conclusiones a escala.
\item Combinar Path-Scanning con perturbacion (Iterated Local Search)
  para explotar el bajo gap inicial del constructor heuristico.
\end{enumerate}
