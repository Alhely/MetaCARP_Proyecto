"""
Variante experimental strict_intra_inter_20260524 para Busqueda Tabu Simple.

Cambios vs. ``run_tabu_simple_p_inter_exp_2026050524.py``:

  1. **Selector binario estricto**: monkey-patch de
     ``seleccionar_grupo_operadores_inter_intra`` dentro de
     ``metacarp.busqueda_tabu_simple`` por la version binaria DETERMINISTA
     de ``metacarp.strict_intra_inter_20260524``.
  ...
  4. **Grid simplificado**: instancia x repeticion. Demas hiperparametros
     (tabu_tenure, tam_vecindario, iteraciones_max, max_iter_sin_mejora)
     usan los defaults del wrapper.

  5. **Salida**: ``experimentos/strict_intra_inter_20260524_tabu_simple/``.
"""
...
SUBCARPETA      = "strict_intra_inter_20260524_tabu_simple"
EXPERIMENTO_TAG = "tabu_simple_strict_intra_inter"
...
def main() -> None:
    correr_grid(
        ...
        descripcion_cli=(
            "Variante experimental strict_intra_inter_20260524 para TS Simple: "
