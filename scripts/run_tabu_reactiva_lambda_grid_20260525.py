"""
Variante experimental lambda_grid_20260525 para Reactive Tabu Search (RTS).

Extiende ``run_tabu_reactiva_strict_intra_inter_20260524`` anadiendo
...
Notas
-----
- RTS NO tiene objetivo penalizado en su bucle principal (usa costo puro).
  ``lambda_capacidad`` afecta solo la seleccion de la solucion inicial
  (``seleccionar_mejor_inicial_rapido``). Las columnas lambda se inyectan
  en el CSV via post-procesado (``inyectar_columnas_lambda``) dado que
  ``extra_csv`` no es volcado a la fila por el nucleo de RTS.
- Salida: ``experimentos/lambda_grid_20260525_tabu_reactiva/``.
"""
...
SUBCARPETA      = "lambda_grid_20260525_tabu_reactiva"
EXPERIMENTO_TAG = "tabu_reactiva_lambda_grid"
...
def _ejecutar_una(tarea: TareaExp) -> tuple[TareaExp, str, dict | None, str | None]:
    ...
    try:
        ...
        res = busqueda_tabu_reactiva_desde_instancia(
            ...
            # Afecta seleccion de solucion inicial; el bucle RTS usa costo puro.
        ...
        # Inyectamos las columnas lambda en el CSV parcial. RTS no vuelca
        # ``extra_csv`` a la fila, asi que sin este paso las columnas no
        # aparecerian en el CSV final.
        inyectar_columnas_lambda(
...
def main() -> None:
    correr_grid(
        ...
        descripcion_cli=(
            "Variante experimental lambda_grid_20260525 para RTS: "
