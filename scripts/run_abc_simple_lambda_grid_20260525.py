"""
Variante experimental lambda_grid_20260525 para ABC Simple.

Extiende ``run_abc_simple_strict_intra_inter_20260524`` anadiendo
...
Notas
-----
- ABC SI consume ``lambda_capacidad`` en su funcion objetivo penalizada.
  ``busqueda_abejas_simple_desde_instancia`` lo acepta como parametro
  publico y ademas escribe la columna ``lambda_capacidad`` en su fila CSV
  nativa. No vuelca ``extra_csv``, por eso el common inyecta a posteriori
  ``lambda_factor`` / ``lambda_actual`` / ``lambda_default``.
- Salida: ``experimentos/lambda_grid_20260525_abc_simple/``.
"""
...
SUBCARPETA      = "lambda_grid_20260525_abc_simple"
EXPERIMENTO_TAG = "abc_simple_lambda_grid"
...
def _ejecutar_una(tarea: TareaExp) -> tuple[TareaExp, str, dict | None, str | None]:
    ...
    try:
        ...
        # Inyectamos las columnas lambda en el CSV parcial. ABC no vuelca
        # ``extra_csv`` a la fila (solo escribe ``lambda_capacidad``), asi
        # que sin este paso las columnas del grid no aparecerian en el CSV.
        inyectar_columnas_lambda(
