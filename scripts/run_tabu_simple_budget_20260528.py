"""
Experimento budget_20260528 para Busqueda Tabu Simple.

Cambios sobre el baseline p_inter_pr_20260528:

  1. PRESUPUESTO x25: ``iteraciones_max`` 400 → 10 000,
     ``max_iter_sin_mejora`` 100 → 2 500 (25 % del nuevo presupuesto).
  2. KICK PROPORCIONAL: ``max_iter_sin_mejora_kick`` 30 → 300 (mismo
     ratio respecto al presupuesto).
  3. SIN TOPE DE RESETS: ``max_resets=None`` (el limite es el budget).
  4. LAMBDA x10: encarece infactibilidad para garantizar convergencia.
...
SUBCARPETA      = "budget_20260528_tabu_simple"
EXPERIMENTO_TAG = "tabu_simple_budget"


def _ejecutar_una(tarea: TareaExp) -> tuple[TareaExp, str, dict | None, str | None]:
    """Worker: una corrida de TS Simple con presupuesto x25 y lambda x10."""
    try:
...
def main() -> None:
    correr_grid(
        ...
        descripcion_cli=(
            "budget_20260528 para TS Simple: iteraciones_max=10000, "
