"""
Variante experimental strict_intra_inter_20260524 para Reactive Tabu Search (RTS).

Cambios vs. ``run_tabu_reactiva_p_inter_exp_2026050524.py``:

  1. **Selector binario estricto**: monkey-patch de
     ``seleccionar_grupo_operadores_inter_intra`` dentro de
     ``metacarp.busqueda_tabu_reactiva`` por la version binaria
     DETERMINISTA de ``metacarp.strict_intra_inter_20260524``.
  ...
  4. **Grid simplificado**: instancia x repeticion. Hiperparametros
     reactivos (tenure dinamico, escape, paciencia) usan defaults
     instance-aware del wrapper.

  5. **Salida**: ``experimentos/strict_intra_inter_20260524_tabu_reactiva/``.
"""
...
SUBCARPETA      = "strict_intra_inter_20260524_tabu_reactiva"
EXPERIMENTO_TAG = "tabu_reactiva_strict_intra_inter"
...
def main() -> None:
    correr_grid(
        ...
        descripcion_cli=(
            "Variante experimental strict_intra_inter_20260524 para RTS: "
